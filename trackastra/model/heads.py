...
class HeadSparseBilinear(nn.Module):
    ...
    # logit for non-neighbour pairs in the scattered dense matrix: large-negative
    # so they vanish under the downstream sigmoid / blockwise-softmax, yet finite
    # (well inside fp16 range) to avoid NaNs in the loss.
    NO_EDGE_LOGIT = -1e4
    ...
    def forward(self, x, y, nbr_idx):
        ...
        return self._scatter_to_dense(scores, nbr_idx, y.shape[1])

    def _scatter_to_dense(self, scores, nbr_idx, m):
        # (B, N, K) neighbour scores -> dense (B, N, M); -1 slots are skipped and
        # topk neighbour indices are unique per row, so there are no collisions.
        b, n, _k = nbr_idx.shape
        A = scores.new_full((b, n, m), self.NO_EDGE_LOGIT)
        valid = nbr_idx >= 0
